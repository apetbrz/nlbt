    ...
    fn from_arg<'a>(
        ...
        Ok(match arg {
            "paycheck" => BC::SetPaycheck {
                amount: budget::parse_dollar_string(vals.next().unwrap())?,
            },
            "paid" => {
                let amount = if let Some(v) = vals.next() {
                    Some(budget::parse_dollar_string(v)?)
                } else {
                    None
                };
                BC::Paid { amount }
            ...
            "edit" => {
                ...
                vals.for_each(|v| {
                    if let Ok(n) = budget::parse_dollar_string(v) {
                        new_amount = Some(n);
                    } else {
    ...
    let budget_commands: BudgetCommands = ["paycheck", "paid", "clear", "edit", "new", "pay"]
        ...
        .flat_map(|id| {
            args.get_occurrences(id).unwrap().filter_map(|arg_iter| {
                BudgetCommand::from_arg(id, arg_iter)
                    .map_err(|e| {
                        println!("{e}");
                        e
                    })
